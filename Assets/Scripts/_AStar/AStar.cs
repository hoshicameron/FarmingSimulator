...
namespace _AStar
{
    public class AStar : MonoBehaviour
    {
        ...
        [Range(0, 20)]
        [SerializeField] private int defaultMovementPenalty = 0;// To favor movement penalty
        ...
        /// <summary>
        /// Builds a path for the given sceneName, from the startGridPosition to the endGridPosition, and adds movement
        /// steps to the passed in npcMovementStack.  Also returns true if path found or false if no path found.
        /// </summary>
        ...
        private void UpdatePathOnNPCMovementStepStack(SceneName sceneName, Stack<NPCMovementStep> npcMovementStepStack)
        {
            ...
            while (nextNode != null) // start doesn't have parent node so while loop will end
            {
        ...
        private void EvaluateCurrentNodeNeighbours(Node currentNode)
        {
            ...
            for (int i = -1; i <= 1; i++)
            {
                ...
                {
                    if (i == 0 && j == 0)
                        continue;    //Position of current node, Ignore it.
                    ...
                    for (int x = 0; x < gridDimensions.x; x++)
                    {
                        for (int y = 0; y < gridDimensions.y; y++)
                        {
                            GridPropertyDetails gridPropertyDetails =
                                GridPropertiesManager.Instance.GetGridPropertyDetails(
                            ...
                            {
                                // If NPC obstacle
                                if (gridPropertyDetails.isNPCObsticle == true)
                                {
