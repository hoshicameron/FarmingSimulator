    ...
    public class NPCMovement : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            if (sceneLoaded)
            {
                if (npcIsMoving == false)
                {
                    ...
                    // If there are step to move then...
                    if (npcPath.npcMovementStepStack.Count > 0)
                    {
                        ...
                        // because we hided NPC so there is no need to move it through the seen just after specific time
                        // we move it to next grid position
                        else
                        {
        ...
        private IEnumerator MoveToGridPositionRoutine(Vector3Int gridPosition, TimeSpan npcMovementStepTime, TimeSpan gameTime)
        {
            ...
            if (npcMovementStepTime > gameTime)
            {
                ...
                //// If speed is less than max speed then move NPC, otherwise skip and move NPC immediately to position
